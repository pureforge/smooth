---
name: smooth-learn
description: "更新持久化的对话记忆。用于捕捉用户偏好、领域玩法和对话踩坑。"
metadata:
  author: smooth
  version: "1.0"
---
...
这通常应由助手主动判断，而不是等用户手动执行命令。只要对话里出现了值得长期保留的协作知识，就可以使用 `smooth-learn` skill。
...
**输入**：如果用户明确给了主题，可以直接用，比如 `stocks`、`response-style`。否则从对话推断。
